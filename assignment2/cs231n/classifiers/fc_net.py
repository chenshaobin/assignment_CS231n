...
class TwoLayerNet(object):
    ...
    def loss(self, X, y=None):
        ...
        # fc1
        fc1_activation, fc1_cache = affine_forward(X, self.params['W1'], self.params['b1'])
        # Relu1
        relu1_activation, relu1_cache = relu_forward(fc1_activation)
        # fc2
        fc2_activation, fc2_cache = affine_forward(relu1_activation, self.params['W2'], self.params['b2'])
        scores = np.copy(fc2_activation)
        ...
        # 反向传播计算梯度
        dout, dw2, db2 = affine_backward(dSoft, fc2_cache)
        dRelu = relu_backward(dout, relu1_cache)
...
class FullyConnectedNet(object):
    ...
    def __init__(
        ...
        for i in range(self.num_layers-1):
            ...
            input_dim = hidden_dims[i]  # 参数的维度会随着隐藏层的单元数的改变而改变
        ...
        if self.normalization == "batchnorm":
            self.bn_params = [{"mode": "train"} for i in range(self.num_layers - 1)]        # 正则化的层数比隐藏层少一层
